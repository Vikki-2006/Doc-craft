"""
PDF Toolkit Pro - Images to PDF View (ui/image_to_pdf.py)
Convert PNG, JPG, JPEG, and BMP images to a single clean PDF document.
Features image re-ordering, preview thumbnails, and layout customization.
"""
...
from utils.helpers import RecentFilesManager, open_folder, format_bytes
...
class ImageToPdfView(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        self.image_list = []  # List of dicts: {'path': str, 'filename': str, 'size': int, 'pil': Image}
    ...
    def _build_toolbar(self):
        """Action toolbar to add images or image folders."""
    ...
    def _build_image_list(self):
        ...
        self._refresh_image_list_ui()

    def _build_footer(self):
        """Bottom options and conversion trigger."""
        ...
        open_chk = ctk.CTkCheckBox(
            ...
            text_color="#94A3B8", checkmark_color="#FFFFFF", fg_color="#8B5CF6"
        )
        open_chk.grid(row=1, column=0, columnspan=2, padx=15, pady=5, sticky="w")

        self.progress_bar = ctk.CTkProgressBar(footer, height=10, fg_color="#0F172A", progress_color="#8B5CF6")
        self.progress_bar.set(0)
        self.progress_bar.grid(row=2, column=0, columnspan=3, padx=15, pady=(10, 5), sticky="ew")

        self.status_label = ctk.CTkLabel(footer, text="Ready to convert images", font=ctk.CTkFont(size=12), text_color="#94A3B8")
        self.status_label.grid(row=3, column=0, columnspan=2, padx=15, pady=(0, 15), sticky="w")

        self.convert_btn = ctk.CTkButton(
            footer, text="🖼️ Create PDF",
        ...
        self.convert_btn.grid(row=3, column=2, padx=15, pady=(0, 15), sticky="e")

    def _add_images(self):
        paths = filedialog.askopenfilenames(
            title="Select Images (PNG, JPG, JPEG, BMP)",
            filetypes=[("Image Files", "*.png;*.jpg;*.jpeg;*.bmp"), ("All Files", "*.*")]
        )
        if paths:
            for p in paths:
                self._load_image_item(p)
            self._refresh_image_list_ui()
            ...
            for root, _, files in os.walk(folder):
                for f in sorted(files):
                    if f.lower().endswith(valid_exts):
                        self._load_image_item(os.path.join(root, f))
                        found = True
            if found:
                self._refresh_image_list_ui()

    def _load_image_item(self, path):
        if any(item['path'] == path for item in self.image_list):
            return
        try:
            ...
            messagebox.showerror("Error", f"Failed to load image '{os.path.basename(path)}': {str(e)}")

    def _refresh_image_list_ui(self):
        ...
        if not self.image_list:
            placeholder = ctk.CTkLabel(
                self.list_container,
                text="No images added yet.\nClick '+ Add Images' above to choose PNG, JPG, JPEG, or BMP files.",
        ...
        for idx, item in enumerate(self.image_list):
            row = ctk.CTkFrame(self.list_container, fg_color="#1E293B", corner_radius=8, border_color="#334155", border_width=1)
            row.pack(fill="x", padx=5, pady=4)
            row.grid_columnconfigure(1, weight=1)

            badge = ctk.CTkLabel(row, text=f"{idx + 1}", width=30, height=30, fg_color="#8B5CF6", text_color="#FFFFFF", corner_radius=6, font=ctk.CTkFont(weight="bold"))
            ...
            sub_lbl = ctk.CTkLabel(info_frame, text=f"Size: {format_bytes(item['size'])}  •  Path: {item['path']}", font=ctk.CTkFont(size=11), text_color="#94A3B8")
    ...
    def _move_item(self, idx, direction):
        ...
        if 0 <= new_idx < len(self.image_list):
            self.image_list[idx], self.image_list[new_idx] = self.image_list[new_idx], self.image_list[idx]
            self._refresh_image_list_ui()
    ...
    def _start_convert_thread(self):
        if self.is_processing:
            return
        if not self.image_list:
            messagebox.showwarning("No Images", "Please add at least one image file.")
            return
        ...
        out_path = os.path.join(out_dir, "Images_Converted.pdf")

        self.is_processing = True
        self.convert_btn.configure(state="disabled", text="Converting...")
        self.progress_bar.set(0)

        threading.Thread(target=self._run_convert, args=(out_path,), daemon=True).start()

    def _run_convert(self, out_path):
        paths = [item['path'] for item in self.image_list]
        ...
        try:
            res_path = images_to_pdf(paths, out_path, progress_callback=progress_cb)
            RecentFilesManager.add_record(self.image_list[0]['filename'], "Images to PDF", res_path)
            self.after(0, lambda: self._on_convert_success(res_path))
        except Exception as e:
            self.after(0, lambda: self._on_convert_error(str(e)))

    def _update_ui_progress(self, progress, message):
        self.progress_bar.set(progress)
        self.status_label.configure(text=message, text_color="#8B5CF6")

    def _on_convert_success(self, res_path):
        self.is_processing = False
        self.convert_btn.configure(state="normal", text="🖼️ Create PDF")
        self.progress_bar.set(1.0)
        self.status_label.configure(text="✅ Images successfully converted to PDF!", text_color="#10B981")
    ...
    def _on_convert_error(self, err_msg):
        self.is_processing = False
        self.convert_btn.configure(state="normal", text="🖼️ Create PDF")
        self.status_label.configure(text=f"❌ Error: {err_msg}", text_color="#EF4444")
        messagebox.showerror("Conversion Failed", f"An error occurred while converting images:\n\n{err_msg}")
